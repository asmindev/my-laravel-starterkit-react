        ...
        >
            ...
            <div className="grid gap-1.5">
                {payload.map((item, index) => {
                    ...
                    return (
                        <div key={item.dataKey ?? index} className={cn('flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5')}>
    ...
    return (
        ...
        >
            {payload.map((item) => {
                ...
                return (
                    <div key={item.value as string} className={cn('flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3')}>
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: unknown, key: string) {
    ...
    } else if ('payload' in payloadObj && typeof payloadObj.payload === 'object' && payloadObj.payload !== null) {
        const nestedPayload = payloadObj.payload as Record<string, unknown>;
